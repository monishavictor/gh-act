...
# existing_pull_requests=$(gh pr list -H $TRAVIS_BRANCH --repo $TRAVIS_REPO_SLUG --state open --limit 1 --label "Feature App")

# if [[ -z "$existing_pull_requests" ]]; then
#     echo "No pull requests found for branch $TRAVIS_BRANCH or release branch with the label Feature App attached."
#     exit 1
# else
#     echo "Pull requests found for branch $TRAVIS_BRANCH with the label Feature App attached."
#     exit 0
# fi
